...
describe('adminQuizGetSessionStatus', () => {
  ...
  test('Token is empty or invalid (does not refer to valid logged in user session): Empty token', () => {
    ...
    const result = adminQuizGetSessionStatus(unavailableToken, quiz1.quizId, quizSession1);
    expect(result).toStrictEqual({
      content: {
        error:
            'Token is empty or invalid (does not refer to valid logged in user session)',
  ...
  test('Session Id does not refer to a valid session within this quiz', () => {
    const result = adminQuizGetSessionStatus(user1, quiz1.quizId, quizSession1 + 1);
    expect(result).toStrictEqual({
      content: {
        error:
  ...
  test('SUCCESS: Quiz with 1 question', () => {
    ...
    const quizTimeLastEdited = adminQuizInfo(user1, quiz1.quizId).content.timeLastEdited;
    expect(result).toStrictEqual({
      content: {
        state: 'LOBBY',
        atQuestion: 0,
          ...
          questions: [
            {
              ...
              answers: [
                ...
                {
                  answerId: adminQuizInfo(user1, quiz1.quizId).content.questions[0].answers[1].answerId,
                  answer: 'Thomas',
                  colour: adminQuizInfo(user1, quiz1.quizId).content.questions[0].answers[1].colour,
                  correct: false,
                },
              ],
            }
  ...
  test('SUCCESS: Quiz with 2 questions', () => {
    ...
    const quizSession2 = adminQuizSessionStart(user1, quiz1.quizId, 5).content.sessionId;
        ...
        metadata: {
          ...
          questions: [
            ...
            },
            {
              ...
              answers: [
                {
                  answerId: adminQuizInfo(user1, quiz1.quizId).content.questions[1].answers[0].answerId,
                  answer: 'Pink',
                  colour: adminQuizInfo(user1, quiz1.quizId).content.questions[1].answers[0].colour,
                  correct: true,
                },
                {
                  answerId: adminQuizInfo(user1, quiz1.quizId).content.questions[1].answers[1].answerId,
                  answer: 'Green',
                  colour: adminQuizInfo(user1, quiz1.quizId).content.questions[1].answers[1].colour,
                  correct: false,
                },
              ],
            }
