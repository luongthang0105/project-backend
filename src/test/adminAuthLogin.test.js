...
describe("adminAuthLogin", () => {
  beforeEach(() => {
    clear()
    let user = adminAuthRegister("javascript@gmail.com", "aikfnrg7", "Java", "Script")
  ...
  test("ERROR: Email address does not exist", () => {
    ...
    expect(error).toStrictEqual({statusCode: 400, error: "Email adress does not exist" })
  })
  ...
  test("Success: 1 person registered", () => {
    ...
    expect(success).toStrictEqual({statusCode: 200, token: expect.any(String)})
  })
  test("Success: More than 1 person registered", () => {
    ...
    expect(user2).toStrictEqual({ statusCode: 200, token: expect.any(String)})
